  ...
  restStream.on("change", (change) => {
    ...
    if (change.operationType === "insert") {
      ...
      console.log(
        ...
        orderKeyLength,
        "updatefield string",
        JSON.stringify(change.updateDescription.updatedFields)
      );

      let result = null;

      // isolates order index from change data
      ...
      if (
        // determines the type of change based on order status, if prep event type is new order
        JSON.stringify(change.updateDescription.updatedFields).includes("prep")
      ) {
        console.log(
          "orderIndex",
          checkLength(),
          "requested order",
          change.fullDocument.activeOrders[parseInt(checkLength())]
        );
// constructs result object with extracted order ID and event type
        result = {
          eventType: "newOrder",
          orderId:
            change.fullDocument.activeOrders[parseInt(checkLength())]._id,
        };
      } else if (
        // determines type of change based on order status if driver on way to collect driver accepted order collection
